...
class Packages extends Component {
  ...
  fetchPackagesList = async () => {
    ...
    const response = await fetch(
      CONSTANT.BaseUrl + 'user/get_packages?user_id=' + id,
    );
    const json = await response.json();
    this.setState({fetchPackages: json, isLoading: false});
  };
  getUser = async () => {
    try {
  ...
  PurchasePackage = async ()=> {
    Alert.alert("Oops" , "Would be available from the next Update")
  //   const {
  //     nonce,
  //     payerId,
  //     email,
  ...
  // );
    ...
    //       Alert.alert('Here', JSON.stringify(response.data.url));
            ...
            <FlatList
              ...
              renderItem={({item, index}) => (
                <View
                  ...
                  <Text style={{flex: 6, fontWeight: '700', color: '#767676'}}>
                    {item.title}:
                  </Text>
                  <Text
                    style={{
